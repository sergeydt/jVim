...
syn match jCondFalse /\%(!\| isnt \|false\)/ display


syn region jCoffeeString start=/"/ skip=/\\\\\|\\"/ end=/"/
syn region jCoffeeString start=/'/ skip=/\\\\\|\\'/ end=/'/
syn match jObjString /['"][a-zA-Z0-9_\-$# ]\+['"][ \t]*:/
syn match jComment /#.*/ contains=@Spell,coffeeTodo


hi def link jCoffeeString String
hi def link jCoffeeComment Comment
...
syn match coffeeKeyword /\<\%(new\|in\|of\|by\|not\|class\|extends\|super\|do\)\>/
\                       display
...
syn match coffeeBoolean /\<\%(on\|yes\|off\|no\)\>/ display
